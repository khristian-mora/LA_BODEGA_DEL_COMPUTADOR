...
def signal_handler(sig, frame):
    global interrupted
    print("\n[INFO] Interrupción recibida, guardando resultados...")
    interrupted = True
...
def main():
    ...
    if offset > 0:
        ...
        print(f"  Offset: {offset} productos omitidos")
    
    # Aplicar límite
    if limit:
        products = products[:limit]
        print(f"  Lote: {len(products)} de {total} (offset: {offset})")
    else:
    ...
    # Determinar nombres de archivos basados en batch_num
    if batch_num is not None:
        json_file = f"productos_enriquecidos_v3_batch{batch_num:02d}.json"
        excel_file = f"LISTADO_ENRIQUECIDO_V3_batch{batch_num:02d}.xlsx"
    else:
    ...
    try:
        ...
        ws_out.title = f"Productos Lote {batch_num}" if batch_num else "Productos Enriquecidos V3"
